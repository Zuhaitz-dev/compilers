    ...
    while (fgets(line, sizeof(line), input))
    {
        ...
        if (1 == data_mode)
        {
            ...
            else if (0 == strcmp(mnemonic, ".STRING"))
            {
                char *start_quote = strchr(p, '"');
                if (!start_quote)
                { /* error already caught in pass 1 */
                    break;
                }
                start_quote++;
                char *end_quote = strchr(start_quote, '"');
        ...
        else // Handle CODE section
        {
            ...
            else
            {
                    ...
                    else
                    {
                        ...
                        if (errno == ERANGE || (*endptr != '\0' && !isspace(*endptr)))
                        {
                            ...
                            else // Must be a label then
                            {
                                ...
                                {
                                    fprintf(stderr,
                                            "Error: Undefined label '%s' at address 0x%04X\n",
                                            operand_str, current_address);
                                    status = EXIT_FAILURE;
                                    instruction_word = encode_format1(OP_ILLEGAL, 0);
                                }
                                else
                                {
                                    ...
                                    else if (OP_LDI == map_entry->opcode ||
                                             OP_LOAD == map_entry->opcode ||
                                             OP_STORE == map_entry->opcode)
                                    {
                                        ...
                                        if (sym->address < REGS.BR)
                                        {
                                            fprintf(
                                                stderr,
                                                "Warning: Data instruction '%s' references code "
                                                "label '%s' at 0x%04X. Using absolute address.\n",
                                    ...
                                    {
                                        fprintf(stderr,
                                                "Error: Label '%s' used with non-data/non-jump "
                                                "instruction '%s' at 0x%04X\n",
                        ...
                        if (0 == instruction_word)
                        {
                            ...
                            if (operand > 4095 || operand < -2048)
                            {
                                ...
                                fprintf(stderr,
                                        "Warning: Operand value '%d' for instruction '%s' at "
                                        "address 0x%04X exceeds 12-bit range. It will be truncated "
                                        "to %u (0x%03X).\n",
                                        operand, mnemonic, current_address, truncated_operand,
                                        truncated_operand);
                            }
                ...
                else // Operand not required
                {
                    ...
                    {
                        fprintf(stderr,
                                "Warning: Operand '%s' for instruction '%s' at address 0x%04X will "
                                "be ignored.\n",
                                operand_str, mnemonic, current_address);
                    }
